...
<script type="text/javascript">
    ...
    function sendData(client_name, client_phone, client_email, client_goods) {
        product_id = client_goods;
        $.ajax({
            type: 'POST',
            data: {
                ref: webmaster_id,
            ...
            success: function (data) {
                ...
                if (data.result == "ok") {
                    ...
                    window.location.replace("lk?lk=" + data.id);
                } else {
    ...
    ?>
        ...
        $('form').submit(function () {
            sendData(get('name'), get('phone'), get('email'), $('input[name=product_id]', this).val()); 
            return false;
        });
    });
</script>